...
{
    int maxpath = 0;
    for (; depth[a] > depth[b]; a = pre[a])
    {
        maxpath = max(maxpath, G[a][pre[a]]);
    }
    for (; depth[a] < depth[b]; b = pre[b])
    {
        maxpath = max(maxpath, G[b][pre[b]]);
    }
    for (; a != b; a = pre[a], b = pre[b])
    {
        maxpath = max(maxpath, max(G[a][pre[a]], G[b][pre[b]]));
    }
    return maxpath;
}
